...
void QSgml::ExportString(QString *HtmlString,char Optimze,int Tabsize)
{
	...
	ExportString(pTag, HtmlString, Optimze, Tabsize);
}

// create a html-file as string
void QSgml::ExportString(QSgmlTag* pTag, QString *HtmlString,char Optimze,int Tabsize)
{
   int iLevelDiff;
   int iDummy;
...
void QSgml::getText(QSgmlTag* pTag, QString* html)
{
	for(int i=0; i < pTag->Children.count(); i++)
	{
		getText(pTag->Children[i], html);
	}

	if (!pTag->Value.isEmpty())
	{
		html->append(pTag->Value);
	}
}

QString QSgml::getText(QSgmlTag *pTag)
{
    QString retval;
    getText(pTag, &retval);
    return retval;
...
{
    QList<QSgmlTag*> retval;
    getElementsByName(Name,AtrName,&retval);
    return retval;
}
...
{
    QList<QSgmlTag*> retval;
    getElementsByName(Name,AtrName,AtrValue,&retval);
    return retval;
}
...
// parse text
bool QSgml::parse(const QString& html)
{
	// delete old elements
	delete DocTag;
	//delete EndTag;
	...
	return(!sSgmlString.isEmpty());
}

QString QSgml::getInnerHtml(QSgmlTag* tag)
{
	int iStart = tag->StartTagPos + tag->StartTagLength;
	int iEnd = tag->EndTagPos;

	return sSgmlString.mid(iStart, iEnd - iStart);
}

QString QSgml::getOuterHtml(QSgmlTag* tag)
{
	int iStart = tag->StartTagPos;
	int iEnd = tag->EndTagPos + tag->EndTagLength;
