...
public:
	...
	//void Encrypt(const std::string & in, std::string & result, int iMode);
	//void Decrypt(const std::string & in, std::string & result, int iMode);

	std::string Encrypt(const std::string & in, int iMode=CBC);
	std::string Decrypt(const std::string & in, int iMode=CBC);

	const QString Encrypt(QString& text)
	{
		ResetChain();
		return QString::fromStdString(Encrypt(text.toStdString()));
	}

	const QString Decrypt(QString& text)
	{
		ResetChain();
		return QString::fromStdString(Decrypt(text.toStdString()));
	}
	...
	bool isKeyInit() const { return m_bKeyInit; }
...
typedef std::shared_ptr<CRijndael> CRijndaelPtr;
